...
// No longer dead: used by the ENDPOINT-CI PATCH in the "Do" operator handler below.
...
struct Processor<'a> {
    _none: PhantomData<&'a ()>,
    // ENDPOINT-CI PATCH (see ../PATCH.md): XObjects already extracted once, document-wide (this
    // Processor is shared across every page -- see output_doc's single `Processor::new()` call
    // outside its per-page loop). A Form XObject's content is fixed regardless of how many times
    // or where it's drawn, so re-extracting a repeated logo/watermark/decorative element never
    // recovers distinct text, only duplicates work -- measured 582 "Do" invocations resolving to
    // far fewer distinct objects on one real PDF.
    visited_xobjects: HashSet<ObjectId>,
    // ENDPOINT-CI PATCH (see ../PATCH.md): fonts already constructed once, document-wide. The
    // original `font_table` in `process_stream` is a fresh, empty HashMap on every call
    // (including every recursive "Do" call), so the SAME font object gets `make_font`'d again
    // in every distinct Form XObject that references it, even though a font's content is just as
    // fixed as a Form's -- this was the actual driver of the RSS spike this patch set out to fix
    // (traced via instrumentation: 607 distinct Form/Image XObjects on one real PDF, most
    // referencing the same handful of real fonts, so make_font ran ~100x more than necessary,
    // each call re-parsing that font's descriptor/widths/embedded program). Keyed by the font
    // object's own reference so it's correct even when different XObjects use different local
    // resource names ("F1"/"F2") for the same underlying font.
    font_cache: HashMap<ObjectId, Rc<dyn PdfFont + 'a>>,
}

impl<'a> Processor<'a> {
    fn new() -> Processor<'a> {
        Processor { _none: PhantomData, visited_xobjects: HashSet::new(), font_cache: HashMap::new() }
    }
        ...
        for operation in &content.operations {
            ...
            match operation.operator.as_ref() {
                ...
                "Tf" => {
                    ...
                    let name = operation.operands[0].as_name().unwrap();
                    // ENDPOINT-CI PATCH (see ../PATCH.md): check the document-level cache (keyed
                    // by the font's own object reference) before falling back to the original
                    // per-stream `font_table` (keyed by local resource name, still needed for
                    // inline/non-referenced font dicts, which have no ObjectId to cache by).
                    let font = match fonts.get(name).and_then(Object::as_reference) {
                        Ok(id) => self
                            .font_cache
                            .entry(id)
                            .or_insert_with(|| make_font(doc, get::<&Dictionary>(doc, fonts, name)))
                            .clone(),
                        Err(_) => font_table.entry(name.to_owned()).or_insert_with(|| make_font(doc, get::<&Dictionary>(doc, fonts, name))).clone(),
                    };
                    {
                ...
                "Do" => {
                    ...
                    // ENDPOINT-CI PATCH (see ../PATCH.md): skip an XObject reference already
                    // extracted once, document-wide. A Form's content is fixed regardless of how
                    // many times/where it's drawn (e.g. a repeated logo/watermark), so skipping a
                    // repeat never loses distinct text, only redundant duplicate work -- measured
                    // 582 "Do" invocations resolving to far fewer distinct objects on one real PDF.
                    if let Ok(id) = xobject.get(name).and_then(Object::as_reference) {
                        if !self.visited_xobjects.insert(id) {
                            continue;
                        }
                    }

                    let xf: &Stream = get(&doc, xobject, name);
                    // ENDPOINT-CI PATCH (see ../PATCH.md): only /Form XObjects are subdocuments
                    // with a content stream to recurse into. /Image XObjects hold raw (often
                    // JPEG/compressed) image bytes -- not PDF drawing commands -- and were
                    // previously fed into process_stream/the content tokenizer unconditionally,
                    // which on image-heavy PDFs causes large, repeated transient-allocation
                    // churn from mis-parsing binary data as PDF syntax (measured: a 5.8MB real
                    // PDF with 53 image Do invocations drove peak RSS to 587MB). Skip anything
                    // explicitly declared /Image; anything else (Form, or no Subtype at all)
                    // keeps the original behaviour.
                    if maybe_get_name_string(&doc, &xf.dict, b"Subtype").as_deref() == Some("Image") {
                        continue;
                    }
